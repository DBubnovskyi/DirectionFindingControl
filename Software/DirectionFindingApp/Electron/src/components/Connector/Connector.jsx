...
export default function Connector() {
    ...
    const { updateRotatorData, updateSettings, registerSendCommand, registerGetSettingsParams } = useRotator();
    const { addLog } = useLogging();

    // Експонуємо addLog в window для доступу з протоколу
    useEffect(() => {
        window.addSerialLog = addLog;
        return () => {
            delete window.addSerialLog;
        };
    }, [addLog]);
    ...
    useEffect(() => {
        ...
        registerGetSettingsParams(() => {
            return protocolRef.current?.getSettingsParameters?.() || [];
        });
    }, [selectedProtocol, updateRotatorData, updateSettings, registerGetSettingsParams]);
        ...
        const loadPorts = async () => {
            if (window.electronAPI) {
                ...
                try {
                    ...
                    // Error loading Web Serial ports
                }
            }
        };
    ...
    return (
        <Stack gap="sm">
            {!window.electronAPI && 'serial' in navigator && !isConnected && (
