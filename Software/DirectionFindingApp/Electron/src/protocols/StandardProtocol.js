...
export class StandardProtocol extends BaseProtocol {
    constructor(updateContext) {
        ...
        this.lastLoggedMessage = ''; // Для логування повних повідомлень
    }
    ...
    parseMessage(message) {
        window.addSerialLog('RX', `${message};`);
        // Додаємо до буфера
        this.buffer += message;
        console.log(`Buffer state: "${this.buffer}"`);
        ...
        console.log(`Parts to parse: [${parts.map(p => `"${p}"`).join(', ')}], remaining buffer: "${this.buffer}"`);
        
        // Зберігаємо розпарсені частини для логування
        const parsedParts = [];
    ...
    /**
     * Повертає список параметрів налаштувань для читання
     * @returns {string[]}
     */
    getSettingsParameters() {
        return ['TOL', 'MINS', 'MAXS', 'BRK'];
    }
}
