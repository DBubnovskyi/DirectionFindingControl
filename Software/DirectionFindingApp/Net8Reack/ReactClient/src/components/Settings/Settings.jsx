...
export default function Settings() {
    const { settings: serverSettings, saveSettingsToServer } = useApp();
    ...
    // Синхронізуємо стан форми з даними сервера
    useEffect(() => {
        setSettings(prev => ({
            ...prev,
            error: serverSettings.tolerance?.toString() || '',
            speedRange: [serverSettings.minSpeed || 0, serverSettings.maxSpeed || 100],
            brakeAngle: serverSettings.brake?.toString() || ''
        }));
    }, [serverSettings]);
    ...
    const handleSave = () => {
        saveSettingsToServer({
            tolerance: parseFloat(settings.error) || serverSettings.tolerance,
            minSpeed: settings.speedRange[0],
            maxSpeed: settings.speedRange[1],
            brake: parseFloat(settings.brakeAngle) || serverSettings.brake
        });
