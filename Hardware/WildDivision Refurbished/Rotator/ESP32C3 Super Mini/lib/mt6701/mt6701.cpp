...
void MT6701::begin(uint8_t sda_pin, uint8_t scl_pin)
{
    ...
    // Перевірка підключення датчика при ініціалізації
    delay(100); // Дати час датчику для готовності
    testConnection();
...
/**
 * @brief Перевіряє чи підключений датчик.
 *
 * @return true якщо датчик підключений, false якщо ні.
 */
...
/**
 * @brief Тестує підключення до датчика.
 *
 * @return true якщо датчик підключений, false якщо ні.
 */
 ...
 *
 * @return Angle in radians withing the range [0, 2*PI) or -1 if sensor not connected.
 */
 ...
 *
 * @return Angle in degrees withing the range [0, 360) or -1 if sensor not connected.
 */
...
int MT6701::readCount()
{
    ...
    uint8_t error = Wire.endTransmission(false); // End transmission, but keep the I2C bus active
    ...
    // Якщо дійшли до цього місця, датчик працює
