...
{
public:
    static constexpr uint8_t DEFAULT_ADDRESS = 0x06;    // I2C address of the MT6701
    ...
    float getRPM();
    void updateCount();
    bool isConnected(); // Перевірка підключення датчика
...
private:
    ...
    std::atomic<bool> sensorConnected{false}; // Стан підключення датчика
    ...
    int readCount();
    bool testConnection(); // Тестування підключення датчика
