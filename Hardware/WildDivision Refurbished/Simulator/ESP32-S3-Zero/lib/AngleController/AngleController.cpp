...
float AngleController::getSensorAngle()
{
    ...
    if (corrected < 0.0f)
        corrected += 360.0f;
// В симуляторі mirrorAngle не потрібен, бо датчик "віртуально" на правильній стороні
...
void AngleController::update()
{
    ...
    if (_isRotating)
    {
        ...
        // Швидкість завжди додатня для відображення
        _currentSpeed = std::min(accelStart, accelEnd) + MinSpeed;
        _currentSpeed = std::min(255.0f, _currentSpeed);

        // Розраховуємо напрямок окремо
        ...
        // Застосовуємо напрямок тільки при передачі в мотор
        _motor.setSpeed(_currentSpeed * direction);
    }
}
...
float AngleController::getTargetAngle()
{
    // Нормалізуємо до 0-360 для відображення
